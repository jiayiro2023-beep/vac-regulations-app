import React, { useState, useMemo, useEffect } from 'react';
import { 
  X, 
  Calculator, 
  CheckCircle2, 
  ChevronRight,
  BookOpen,
  RefreshCw
} from 'lucide-react';

interface CalculatorModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSelectRegulation: (regId: string) => void;
}

interface MonthBlock {
  monthIndex: number;
  startStr: string;
  endStr: string;
  totalDays: number;
  validDays: number;
  status: '足月' | '未足月' | '逾期';
  rate: number;
  amount: number;
}

export const CalculatorModal: React.FC<CalculatorModalProps> = ({
  isOpen,
  onClose,
  onSelectRegulation
}) => {
  const [allowanceType, setAllowanceType] = useState<'訓後就業津貼' | '推介就業津貼'>('訓後就業津貼');
  const [startDate, setStartDate] = useState<string>('2026-03-15');
  const [expiryDate, setExpiryDate] = useState<string>('2026-06-10');
  const [selectedMonths, setSelectedMonths] = useState<Record<number, boolean>>({
    1: true,
    2: true,
    3: true
  });

  // Reset selected months when start/expiry dates change to select all eligible months by default
  useEffect(() => {
    // Select months 1 to 12 if they have any valid days
    const newSelected: Record<number, boolean> = {};
    const blocks = calculateBlocks(allowanceType, startDate, expiryDate);
    blocks.forEach(b => {
      if (b.status !== '逾期') {
        newSelected[b.monthIndex] = true;
      }
    });
    setSelectedMonths(newSelected);
  }, [allowanceType, startDate, expiryDate]);

  // Core Math Calculation
  function calculateBlocks(
    type: '訓後就業津貼' | '推介就業津貼', 
    startStr: string, 
    expiryStr: string
  ): MonthBlock[] {
    const blocks: MonthBlock[] = [];
    if (!startStr || !expiryStr) return blocks;

    const start = new Date(startStr);
    const expiry = new Date(expiryStr);

    for (let m = 0; m < 12; m++) {
      // Month start
      const blockStart = new Date(start);
      blockStart.setMonth(start.getMonth() + m);

      // Month end
      const blockEnd = new Date(start);
      blockEnd.setMonth(start.getMonth() + m + 1);
      blockEnd.setDate(blockEnd.getDate() - 1);

      const totalDays = Math.round((blockEnd.getTime() - blockStart.getTime()) / (1000 * 60 * 60 * 24)) + 1;
      
      // Determine valid days within guidance
      let validDays = 0;
      let status: '足月' | '未足月' | '逾期' = '逾期';

      if (expiry >= blockEnd) {
        validDays = totalDays;
        status = '足月';
      } else if (expiry >= blockStart) {
        validDays = Math.round((expiry.getTime() - blockStart.getTime()) / (1000 * 60 * 60 * 24)) + 1;
        status = '未足月';
      } else {
        validDays = 0;
        status = '逾期';
      }

      // Determine rate
      let rate = 4000;
      if (type === '訓後就業津貼') {
        rate = (m < 6) ? 6000 : 4000;
      }

      const amount = status === '足月' 
        ? rate 
        : status === '未足月' 
          ? Math.round(rate * (validDays / totalDays)) 
          : 0;

      blocks.push({
        monthIndex: m + 1,
        startStr: formatDate(blockStart),
        endStr: formatDate(blockEnd),
        totalDays,
        validDays,
        status,
        rate,
        amount
      });
    }

    return blocks;
  }

  // Helper: format Date object to YYYY-MM-DD
  function formatDate(d: Date): string {
    const year = d.getFullYear();
    const month = String(d.getMonth() + 1).padStart(2, '0');
    const day = String(d.getDate()).padStart(2, '0');
    return `${year}-${month}-${day}`;
  }

  const blocks = useMemo(() => {
    return calculateBlocks(allowanceType, startDate, expiryDate);
  }, [allowanceType, startDate, expiryDate]);

  // Total summary based on selection
  const summary = useMemo(() => {
    let total = 0;
    let fullMonthsCount = 0;
    const partialMonthsDetails: string[] = [];
    const formulaParts: string[] = [];

    blocks.forEach(b => {
      if (selectedMonths[b.monthIndex]) {
        total += b.amount;
        if (b.status === '足月') {
          fullMonthsCount += 1;
        } else if (b.status === '未足月') {
          partialMonthsDetails.push(
            `第 ${b.monthIndex} 個月：${b.rate.toLocaleString()} 元 × (${b.validDays}/${b.totalDays} 天) = ${b.amount.toLocaleString()} 元`
          );
        }
      }
    });

    if (fullMonthsCount > 0) {
      const rateLabel = allowanceType === '訓後就業津貼' ? '訓後就業基準' : '4,000 元';
      formulaParts.push(`足月部分：${rateLabel} × ${fullMonthsCount} 個月`);
    }
    blocks.forEach(b => {
      if (selectedMonths[b.monthIndex] && b.status === '未足月') {
        formulaParts.push(`未足月部分：${b.rate.toLocaleString()} 元 × (${b.validDays}/${b.totalDays} 天)`);
      }
    });

    const calculationFormula = formulaParts.length > 0 
      ? `${formulaParts.join(' + ')} ≈ ${total.toLocaleString()} 元`
      : '未勾選任何有效月份';

    return {
      total,
      fullMonthsCount,
      partialMonthsDetails,
      calculationFormula
    };
  }, [blocks, selectedMonths, allowanceType]);

  if (!isOpen) return null;

  // Preset Loaders
  const loadPreset = (caseNum: 1 | 2) => {
    if (caseNum === 1) {
      setAllowanceType('訓後就業津貼');
      setStartDate('2026-03-15');
      setExpiryDate('2026-06-10');
    } else {
      setAllowanceType('推介就業津貼');
      setStartDate('2026-03-01');
      setExpiryDate('2026-05-10');
    }
  };

  const handleToggleMonth = (idx: number) => {
    setSelectedMonths(prev => ({
      ...prev,
      [idx]: !prev[idx]
    }));
  };

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/65 p-2 backdrop-blur-sm animate-fadeIn no-print sm:p-4">
      <div className="flex max-h-[96vh] w-full max-w-3xl flex-col overflow-hidden rounded-[24px] border border-[#e3dcce] bg-white shadow-[0_24px_80px_rgba(0,0,0,0.25)] dark:border-slate-800 dark:bg-slate-900 sm:max-h-[92vh] sm:rounded-[30px]">
        
        {/* Modal Header */}
        <div className="flex flex-shrink-0 items-center justify-between gap-2 bg-gradient-to-r from-[#102b4a] via-[#14375d] to-[#0d233c] px-4 py-4 text-white sm:px-6 sm:py-5">
          <div className="flex items-center gap-2.5 min-w-0">
            <div className="flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-2xl border border-blue-200/20 bg-white/10 text-blue-200 sm:h-10 sm:w-10">
              <Calculator className="w-4 h-4 sm:w-5 sm:h-5" />
            </div>
            <div className="min-w-0">
              <h2 className="text-sm sm:text-base font-bold leading-tight">
                <span className="sm:hidden">第二類退役官兵未足月津貼</span>
                <span className="hidden sm:inline">第二類退除役官兵未足月津貼計算機</span>
              </h2>
              <p className="text-xs text-blue-200">穩定就業津貼發給辦法第 5 條</p>
            </div>
          </div>
          <button
            onClick={onClose}
            className="p-1.5 rounded-lg text-slate-300 hover:text-white hover:bg-white/10 transition-colors flex-shrink-0"
          >
            <X className="w-5 h-5" />
          </button>
        </div>

        {/* Modal Scrollable Content */}
        <div className="flex-1 space-y-5 overflow-y-auto px-3 py-4 text-slate-800 dark:text-slate-200 sm:space-y-6 sm:p-6">
          
          {/* Quick Presets Banner */}
          <div className="space-y-2 rounded-2xl border border-[#e3dcce] bg-[#faf8f3] p-3 dark:border-slate-800 dark:bg-slate-900/60 sm:p-4">
            <div className="flex items-center gap-1.5 text-[13px] font-semibold text-[#635c4e] dark:text-slate-300">
              <RefreshCw className="w-3.5 h-3.5 text-[#1b4d82] flex-shrink-0" />
              <span>載入官方釋例預設值</span>
            </div>
            {/* Preset buttons — always stacked vertically on mobile */}
            <div className="flex flex-col sm:flex-row gap-2">
              <button
                onClick={() => loadPreset(1)}
                className="w-full text-[13px] font-bold px-3 py-2.5 bg-[#edf3fb] hover:bg-[#dfeaf8] text-[#1b4d82] rounded-xl transition-all border border-[#c8d8ea] text-left dark:bg-blue-950/40 dark:hover:bg-blue-900/40 dark:text-blue-300 dark:border-blue-800/50"
              >
                案例一：訓後就業 (3/15投保, 6/10滿期)
              </button>
              <button
                onClick={() => loadPreset(2)}
                className="w-full text-[13px] font-bold px-3 py-2.5 bg-[#eeedfa] hover:bg-[#e2e0f8] text-[#5243aa] rounded-xl transition-all border border-[#d8d4f2] text-left dark:bg-indigo-950/40 dark:hover:bg-indigo-900/40 dark:text-indigo-300 dark:border-indigo-800/50"
              >
                案例二：推介就業 (3/1投保, 5/10滿期)
              </button>
            </div>
          </div>

          {/* Parameters Group */}
          <div className="grid grid-cols-1 gap-4 font-sans sm:grid-cols-2">
            {/* Input Left: Type & Dates */}
            <div className="space-y-4">
              <div className="space-y-2">
                <label className="block text-[13px] font-bold text-[#635c4e] dark:text-slate-400 uppercase tracking-wider">
                  穩定就業津貼項目
                </label>
                <div className="flex gap-2">
                  {(['訓後就業津貼', '推介就業津貼'] as const).map(type => (
                    <button
                      key={type}
                      onClick={() => setAllowanceType(type)}
                      className={`flex-1 py-2 px-3 rounded-xl border text-[13px] font-bold text-center transition-all ${
                        allowanceType === type
                          ? 'bg-[#1b4d82] text-white border-[#1b4d82] shadow-sm'
                          : 'bg-[#faf8f3] text-slate-700 border-[#e3dcce] hover:bg-[#ede6d4] dark:bg-slate-800/80 dark:text-slate-300 dark:border-slate-700 dark:hover:bg-slate-700/60'
                      }`}
                    >
                      {type}
                    </button>
                  ))}
                </div>
              </div>

              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1.5">
                  <label className="block text-[13px] font-bold text-[#635c4e] dark:text-slate-400 uppercase tracking-wider">
                    投保就業開始日
                  </label>
                  <input
                    type="date"
                    value={startDate}
                    onChange={(e) => setStartDate(e.target.value)}
                    className="w-full px-3 py-2 bg-[#faf8f3] border border-[#e3dcce] text-slate-900 dark:bg-slate-800/60 dark:border-slate-700 dark:text-slate-100 rounded-xl text-[14px] font-bold focus:outline-none focus:ring-2 focus:ring-[#1b4d82]"
                  />
                </div>
                <div className="space-y-1.5">
                  <label className="block text-[13px] font-bold text-[#635c4e] dark:text-slate-400 uppercase tracking-wider">
                    輔導期限屆滿日
                  </label>
                  <input
                    type="date"
                    value={expiryDate}
                    onChange={(e) => setExpiryDate(e.target.value)}
                    className="w-full px-3 py-2 bg-[#faf8f3] border border-[#e3dcce] text-slate-900 dark:bg-slate-800/60 dark:border-slate-700 dark:text-slate-100 rounded-xl text-[14px] font-bold focus:outline-none focus:ring-2 focus:ring-[#1b4d82]"
                  />
                </div>
              </div>
            </div>

            {/* Input Right: Info Box */}
            <div className="p-3 sm:p-4 bg-[#faf8f3] border border-[#e3dcce] dark:bg-slate-800/30 dark:border-slate-700/50 rounded-2xl flex flex-col justify-center space-y-2">
              <h4 className="flex items-center gap-1.5 text-[14px] font-bold text-slate-800 dark:text-slate-200">
                <BookOpen className="w-4 h-4 text-[#1b4d82] flex-shrink-0" />
                <span>比例計算說明</span>
              </h4>
              <p className="text-[13px] leading-6 text-[#635c4e] dark:text-slate-300 break-words">
                第二類退除役官兵超過輔導期限不予發給，輔導期限內未足月之就業期間依日數按比率計算，四捨五入至 1 元。
              </p>
            </div>
          </div>

          {/* Month Range Select Grid */}
          <div className="space-y-2">
            {/* Header: label + reset button — allow wrap */}
            <div className="flex items-start justify-between gap-2 pl-1 flex-wrap">
              <label className="block text-[13px] font-bold text-[#635c4e] dark:text-slate-400 uppercase tracking-wider leading-snug">
                選擇欲核發之就業月份
              </label>
              <button 
                onClick={() => {
                  const allOn: Record<number, boolean> = {};
                  blocks.forEach(b => {
                    if (b.status !== '逾期') allOn[b.monthIndex] = true;
                  });
                  setSelectedMonths(allOn);
                }}
                className="text-[13px] font-bold text-[#1b4d82] hover:underline whitespace-nowrap"
              >
                重設建議申辦月份
              </button>
            </div>
            
            <div className="overflow-hidden rounded-2xl border border-[#e3dcce] text-[14px] dark:border-slate-800">
              {blocks.map((b) => {
                const isSelected = !!selectedMonths[b.monthIndex];
                const isEligible = b.status !== '逾期';
                
                return (
                  <div 
                    key={b.monthIndex} 
                    className={`p-3 transition-colors ${
                      !isEligible 
                        ? 'bg-[#f4efe4] opacity-50 dark:bg-slate-900/10' 
                        : isSelected 
                          ? 'bg-[#eef4fb] dark:bg-blue-950/20' 
                          : 'bg-white dark:bg-slate-900'
                    }`}
                  >
                    {/* Top row: checkbox + month label + dates */}
                    <div className="flex items-center justify-between gap-2">
                      <div className="flex items-center gap-2.5 min-w-0">
                        <input
                          type="checkbox"
                          checked={isSelected && isEligible}
                          disabled={!isEligible}
                          onChange={() => handleToggleMonth(b.monthIndex)}
                          className="rounded text-[#1b4d82] focus:ring-[#1b4d82] disabled:opacity-50 w-4 h-4 flex-shrink-0"
                        />
                        <span className="font-bold text-slate-800 dark:text-slate-100 whitespace-nowrap">
                          第 {b.monthIndex} 個月
                        </span>
                        <span className="text-xs text-[#7d7768] font-mono truncate">
                          {b.startStr.slice(5)} ~ {b.endStr.slice(5)}
                        </span>
                      </div>
                      {/* Amount — always visible */}
                      <span className={`font-mono font-bold text-right flex-shrink-0 ${
                        b.amount > 0 ? 'text-slate-800 dark:text-slate-100' : 'text-slate-400 dark:text-slate-600'
                      }`}>
                        {b.amount > 0 ? `NT$ ${b.amount.toLocaleString()}` : '—'}
                      </span>
                    </div>

                    {/* Status badge on its own line — always readable */}
                    <div className="mt-1.5 ml-6">
                      <span className={`inline-block rounded px-2 py-0.5 text-xs font-bold ${
                        b.status === '足月'
                          ? 'bg-emerald-100 dark:bg-emerald-950/80 text-emerald-800 dark:text-emerald-300'
                          : b.status === '未足月'
                            ? 'bg-amber-100 dark:bg-amber-950/80 text-amber-800 dark:text-amber-300 border border-amber-200 dark:border-amber-800/60'
                            : 'bg-slate-100 text-slate-500 dark:bg-slate-800 dark:text-slate-500 border border-slate-200/40 dark:border-slate-700/40'
                      }`}>
                        {b.status === '足月' ? '足月' : b.status === '未足月' ? `未足月 (${b.validDays}/${b.totalDays} 天)` : '已逾輔導期'}
                      </span>
                    </div>
                  </div>
                );
              })}
            </div>
          </div>

          {/* Results Block */}
          <div className="flex-shrink-0 space-y-3 rounded-[22px] bg-gradient-to-br from-[#102b4a] via-[#14375d] to-[#0d233c] p-4 text-white shadow-[0_14px_32px_rgba(16,43,74,0.2)] sm:space-y-4 sm:p-5">
            {/* Header row */}
            <div className="flex items-center justify-between border-b border-indigo-950/80 pb-3 gap-2">
              <div className="flex items-center gap-2">
                <CheckCircle2 className="w-4 h-4 sm:w-5 sm:h-5 text-emerald-400 flex-shrink-0" />
                <span className="text-[14px] font-bold sm:text-[15px]">穩定就業津貼試算結果</span>
              </div>
              <span className="px-2 py-0.5 rounded-full text-xs font-bold bg-emerald-500/20 text-emerald-300 border border-emerald-500/30 whitespace-nowrap">
                試算成功
              </span>
            </div>

            {/* Amount — large and prominent */}
            <div>
              <div className="text-xs font-semibold uppercase tracking-wide text-indigo-200">核發津貼總金額</div>
              <div className="text-2xl sm:text-3xl font-black text-emerald-400 mt-1">
                NT$ {summary.total.toLocaleString()}
              </div>
            </div>

            {/* Formula */}
            <div>
              <div className="mb-1.5 text-xs font-semibold uppercase tracking-wide text-indigo-200">核算公式明細</div>
              <div className="text-xs font-mono text-slate-300 break-all overflow-wrap-anywhere bg-white/5 p-2.5 rounded-lg border border-white/10 leading-relaxed">
                {summary.calculationFormula}
              </div>
            </div>

            {/* Partial month details */}
            {summary.partialMonthsDetails.map((det, idx) => (
              <p key={idx} className="text-[13px] text-slate-300 leading-relaxed bg-white/5 px-3 py-2 rounded-xl border border-white/5 font-mono">
                未足月計算明細：{det}
              </p>
            ))}

            {/* Regulation link — stacks on mobile */}
            <div className="pt-2 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 text-xs border-t border-white/10">
              <span className="flex items-center gap-1 text-[13px] text-slate-400">
                <BookOpen className="w-3.5 h-3.5 flex-shrink-0" />
                <span>依據：穩定就業津貼發給辦法第 5 條</span>
              </span>
              <button
                onClick={() => {
                  onSelectRegulation('促進退除役官兵穩定就業津貼發給辦法作業說明_114_12_24修_含附件___1__pdf');
                  onClose();
                }}
                className="flex items-center gap-1 text-blue-300 hover:text-white font-semibold transition-colors self-start sm:self-auto"
              >
                <span>查看作業說明問題一</span>
                <ChevronRight className="w-4 h-4" />
              </button>
            </div>
          </div>

        </div>

        {/* Modal Footer */}
        <div className="flex flex-shrink-0 justify-end border-t border-[#e3dcce] bg-[#faf8f3] px-3 py-3 dark:border-slate-700 dark:bg-slate-800/80 sm:px-6 sm:py-4">
          <button
            onClick={onClose}
            className="rounded-xl bg-[#ede6d4] px-4 py-2.5 text-[13px] font-bold text-slate-800 transition-colors hover:bg-[#e0d6be] dark:bg-slate-700 dark:text-slate-200 dark:hover:bg-slate-600"
          >
            關閉試算視窗
          </button>
        </div>
      </div>
    </div>
  );
};
